...
namespace ShowGo
{
    ...
    public partial class VcamBehaviour : NetworkBehaviour
    {
        ...
        void OnSyncedVCamChanged(int oldIndex, int newIndex)
        {
            ...
            {
                DJTimer = 10f;
                StartCoroutine(CameraStateEnd(newIndex));
            }
        ...
        IEnumerator CameraStateEnd(int index)
        {
            if (3 < index && index < 6) { yield return new WaitForSeconds(20); }
            if (0 < index && index < 4) { yield return new WaitForSeconds(10); }
            ...
            if (LocalTimer <= 0 && vCams[index].Priority == 30)
            {
                ...
                if(!(FireworkBehaviour.instance.isFireworking && PreviousVC == vCams[4]))vCams[index].Priority -= 20;//意思就是说，每切换一次镜头就重置一次计时器，如果计时器用完了且当前镜头和协程要降低优先级的镜头是同一个，就说明这个镜头要被自动降级了因为他呆的时间太久了；那么就是说我们去手动切镜头的时候还是要给老镜头降级的，但是lookaround应该是个例外，它优先级一直不变
            }
            PreviousVC = FindPreviousVC();
        }

        public async void LocalCameraChange(int index)
        {
            PreviousVC = FindPreviousVC();
            if (PreviousVC == vCams[5]) { FloatingUiConnector.Instance.ClosePuppyCard(); }
            ...
            {
                LocalTimer = 20f;
                StartCoroutine(CameraStateEnd(index));
            }
