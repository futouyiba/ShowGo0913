...
public class FloatingUiConnector : SingletonMono<FloatingUiConnector>
{
    ...
    public void SpawnChatBubble(uint userId, string msg)
    {
        ...
        if (!SceneShowgoMgr.Instance.Me.isDJ)
        {
            if (msg != "定位自己")
            {
                if (userId == SceneShowgoMgr.Instance.Me.UserId) VcamBehaviour.instance.LocalCameraChange(4, duration + 0.1f);
            }
    ...
    public void SpawnChatBubble(uint userId, string msg,bool isEverybodySpeak)//草，everybodyspeak镜头不动
    {
        // done @lizhaozhao
        if (!chatBubbles.ContainsKey(userId))
        {
