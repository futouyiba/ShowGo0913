...
namespace ShowGo
{
    ...
    public partial class ShowGoPlayer : NetworkBehaviour
    {
        ...
        /// </summary>
        [SyncVar(hook = "DJChanged")] public bool isDJ = false;
        void DJChanged(bool oldDJ, bool newDJ)
        {
            InterrupteMove();
            //20221010DJ切换行为
            if (oldDJ)
            {
                ...
                SceneShowgoMgr.Instance.currentDJSince = -1;
            }
            if (newDJ)
            {
                transform.position = SceneShowgoMgr.Instance.DJSpot.position;
                transform.localScale = new Vector3(2.7f, 2.7f, 1);
                //SceneShowgoMgr.Instance.currentDJ = (int)UserId;
                SceneShowgoMgr.Instance.currentDJSince = DateTime.UtcNow.Millisecond;
                if (isLocalPlayer)
                {
                    string DJLine = "全体目光向我看齐，我宣布个事儿，我是DJ！";
                    SendChatMsg(UserId, DJLine);
                    VcamBehaviour.instance.CmdSeverCameraChange(1, 3f);
                }
            }
        }

        [Command]public void CmdShowCat(bool open)
        {
            RpcShowCat(open);
        }

        [ClientRpc]public void RpcShowCat(bool open)
        {
            SceneShowgoMgr.Instance.DJCat.SetActive(open);
        }
        ...
        public void InterrupteMove()//20201010为了DJ打断移动
        {
            if (!(_moveTweenerCore.IsActive() && _moveTweenerCore.IsPlaying())) { return; }
        ...
        public async void OnAprcIdChanged(int oldAprc, int newAprc)
        {
            ...
            // var asset = await AssetCache.Instance.GetConfig<ShowGoPlayerCfg>(EConfig.ShowGoPlayerCfg);
            ...
            if (!aprcObj)
            {// no aprc obj, create new
                var cfg = await AssetCache.Instance.GetConfig<XGAprcCfg>(EConfig.XGAprcCfg);
                if(!cfg.characterPrefab) Debug.LogError($"character prefab does not exist");
                void CreateAprc()
                {
                    GameObject aprcCreated = Instantiate(cfg.characterPrefab, transform);
                    var aprcCtrl = aprcCreated.GetComponent<XGAprcCtrl>();
                    aprcCtrl.LvChanged(newAprc);
                    aprcObj = aprcCreated;
                }
                SceneShowgoMgr.Instance.AddCreateTask(CreateAprc);
            }
            else
            {//already aprc obj, update
                var aprcCtrl = aprcObj.GetComponent<XGAprcCtrl>();
                aprcCtrl.LvChanged(newAprc);
            }
        ...
        public async void OnRoomLvChanged(int oldLv, int NewLv)
        {
            if (!aprcObj)
            {// no aprc obj, create new
                var cfg = await AssetCache.Instance.GetConfig<XGAprcCfg>(EConfig.XGAprcCfg);
        ...
        private async void CheckClientPrefix(string msg)
        {
            ...
            var cfg = await AssetCache.Instance.GetConfig<ShowGoPlayerCfg>(EConfig.ShowGoPlayerCfg); 
            foreach (var kvPair in cfg.clientCommandPrefixes)
            {
    ...
    public partial class ShowGoPlayer
    {
        ...
        {
            var cfg = await AssetCache.Instance.GetConfig<ShowGoPlayerCfg>(EConfig.ShowGoPlayerCfg); 
            foreach (var kvPair in cfg.commandPrefixes)
            {
                if (msg.StartsWith(kvPair.Key))
                {
                    var driedMsg = msg.TrimStart(kvPair.Key.ToCharArray());
        ...
        public void SvrPreDJ_Movie(string text)
        {
            if(!SceneShowgoMgr.Instance.AmIDJ((int)UserId)) return;
            //throw new NotImplementedException();
            else { VcamBehaviour.instance.CmdSeverCameraChange(2, 10f); }
        }
        ...
        public void SvrPreDJ_EverybodySay(string text)
        {
            ...
            if(!SceneShowgoMgr.Instance.AmIDJ((int)UserId)) return;
            CmdEveryoneSpeak(text);
        }

        [Command(requiresAuthority =false)]
        public void CmdEveryoneSpeak(string text)
        {
            RpcEveryoneSpeak(text);
        }
        [ClientRpc]
        public void RpcEveryoneSpeak(string text)
        {
        ...
        public void PrefixMove(string driedMsg)
        {
            ...
            if (isDJ) { Debug.Log("DJ is not allowed to move voluntarily. Stick your ass to the stage!"); _fsm.TriggerUnityEvent("MoveFailed"); return;  }
        ...
        public async void ftOnEnterIdle()
        {
            ...
            //get rand pos
            //set dest
            if (isDJ) { Debug.Log("DJ is not allowed to move randomly. Stick your ass to the stage!"); _fsm.TriggerUnityEvent("MoveFailed"); return;  }//不加这个DJ换人后卡死在idle状态
            var randPos = PartyBehaviour.Instance.GetRandomWalkablePosition();
            SetDestination(randPos);
        }
    ...
    public partial class ShowGoPlayer
    {
        async void Start()
        {
            if (isLocalPlayer)
            {
                ...
                VcamBehaviour.instance.LocalCameraChange(4, 4.5f);//20220923进场先看自己
