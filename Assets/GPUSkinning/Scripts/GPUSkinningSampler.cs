...
public class GPUSkinningSampler : MonoBehaviour 
{
	...
	private SkinnedMeshRenderer[] renderList;

	// private List<GPUSkinningAnimation> skinningAnimationList = new List<GPUSkinningAnimation>();
    private GPUSkinningAnimation gpuSkinningAnimation = null;

     
    // private List<GPUSkinningClip> skinningClipList = new List<GPUSkinningClip>();
    ...
    public void StartSample()
	{
		...
		// 	return;
		// }
        // skinningAnimationList
        foreach (var meshRenderer in renderList)
        {
            StartSampleRender(meshRenderer, numFrames);
        }
    ...
    private void StartSampleRender(SkinnedMeshRenderer smr, int numFrames)
    {
        ...
        boneAmount = gpuSkinningAnimation.bones.Length;
    ...
    private GameObject CreatePlayerPrefab(string assetPath, List<Mesh> meshList, List<Material> materialList, List<TextAsset> testAssetList)
    {
        // string savedPrefabPath = dir + "/GPUSKinning_Prefab_" + animName + "_" + ".prefab";
        GameObject root = new GameObject();
        root.name = "GPUSKinning_" + gameObject.name;

        for (int i = 0; i < renderList.Length; i++)
        {
            var smr = renderList[i];
            var mesh = meshList[i];
            Debug.Log(i);
            var mat = materialList[i];
            var textAsset = testAssetList[i];
            
            GameObject render = new GameObject(smr.name);
            render.transform.parent = root.transform;
            render.transform.localPosition = Vector3.zero;
            render.transform.localRotation = Quaternion.identity;
            render.transform.localScale = Vector3.one;
            
            var meshRenderer = render.AddComponent<MeshRenderer>();
            var skinningPlayer = render.AddComponent<GPUSkinningPlayerMono>();
            
            skinningPlayer.Init(gpuSkinningAnimation,mesh, mat, textAsset);
        }
        return root;
    }

    private Mesh CreateNewMesh(SkinnedMeshRenderer smr, Mesh mesh, string meshName)
    {
        ...
        for(int i = 0; i < numVertices; ++i)
        {
            ...
            // Add the code below to fix this bug
            ...
            {
                weights[1].weight = 1.0f - weights[0].weight;
            }
    ...
    private void CreateTextureMatrix(string dir, GPUSkinningAnimation gpuSkinningAnim, SkinnedMeshRenderer smr, List<TextAsset> assetList)
    {
        ...
        for (int clipIndex = 0; clipIndex < gpuSkinningAnim.clips.Length; ++clipIndex)
        {
            ...
            for (int frameIndex = 0; frameIndex < numFrames; ++frameIndex)
            {
                ...
                for (int matrixIndex = 0; matrixIndex < numMatrices; ++matrixIndex)
                {
                    ...
                    if (matrixIndex < gpuSkinningAnim.bones.Length)
                    {
                        var bone = gpuSkinningAnim.bones[matrixIndex];
                        var poseIndex = System.Array.IndexOf(smr.bones, bone.transform);
                        if (poseIndex != -1 && poseIndex < smr.sharedMesh.bindposes.Length)
                        {
                            bindpose = smr.sharedMesh.bindposes[poseIndex];
                        }
                    }
                    
                    Matrix4x4 matrix = matrices[matrixIndex] * bindpose;
        ...
        string savedPath = dir + "/GPUSKinning_Texture_" + animName + "_" + smr.name + ".bytes";
        ...
        assetList.Add(AssetDatabase.LoadAssetAtPath<TextAsset>(savedPath));
        ...
        if (samplingFrameIndex >= totalFrams)
        {
			...
			if(!string.IsNullOrEmpty(savePath))
			{
				...
				else
				{
                    ...
                    {
                        CreateTextureMatrix(dir, gpuSkinningAnimation, smr, textAssetList);
                    }
                    ...
                    if (samplingClipIndex == 0)
                    {
                        foreach (var smr in renderList)
                        {
                            Mesh newMesh = CreateNewMesh(smr, smr.sharedMesh, "GPUSkinning_Mesh");
                            if (savedMesh != null)
                            {
                                newMesh.bounds = savedMesh.bounds;
                            }
                            meshList.Add(newMesh);
                            string savedMeshPath = dir + "/GPUSKinning_Mesh_" + animName + "_" + smr.name + ".asset";
                            AssetDatabase.CreateAsset(newMesh, savedMeshPath);
                            WriteTempData(TEMP_SAVED_MESH_PATH, savedMeshPath);
                            savedMesh = newMesh;

                            CreateShaderAndMaterial(smr, dir, matList);

                            CreateLODMeshes(newMesh.bounds, dir);
                        }
                    }

                    string savedPrefabPath = dir + "/bakedprefab" + animName  + ".prefab";
                    var prefab = CreatePlayerPrefab(dir, meshList, matList, textAssetList);
                    PrefabUtility.SaveAsPrefabAsset(prefab, savedPrefabPath);
	...
	private void CreateShaderAndMaterial(SkinnedMeshRenderer smr, string dir, List<Material> matList)
	{
        Shader shader = null;
        if (createNewShader)
        {
        ...
        else
        {
            ...
            Debug.Log("shaderName:" + shaderName + " shader:" + shader);
		...
		string savedMtrlPath = dir + "/GPUSKinning_Material_" + animName + "_" + smr.name + ".mat";
        matList.Add(mtrl);
